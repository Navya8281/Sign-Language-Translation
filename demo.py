...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    ...
    while st.session_state.camera_active:
        ...
        if len(sequence) == 30 and (np.any(left_hand) or np.any(right_hand)):
            ...
            confidence = np.max(res)  # Confidence score
            predicted_word = actions[np.argmax(res)] if confidence > threshold else ""
        else:
            predicted_word = ""
            confidence = 0.0
        ...
        predicted_sign.markdown(
            f"<h3 class='predicted-sign'>Predicted Sign: {predicted_word} (Confidence: {confidence:.2f})</h3>", 
            unsafe_allow_html=True
        )
